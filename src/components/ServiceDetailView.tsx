...
import DigitalMarketingPackages from './DigitalMarketingPackages';
...
export default function ServiceDetailView({ serviceId, setActivePage, language = 'en' }: ServiceDetailViewProps) {
    ...
    <article className="pt-40 md:pt-48 pb-16 bg-brand-black min-h-screen text-[var(--white)] relative overflow-hidden">
      ...
      {serviceId === 'digital-marketing' ? (
        <div className="relative z-10">
          <DigitalMarketingPackages language={language} setActivePage={setActivePage} />
        </div>
      ) : (
        <div className="relative z-10 max-w-7xl mx-auto px-6 py-10 grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <div className="flex flex-col gap-6 lg:h-max lg:sticky lg:top-32">
            ...
            <div className="bg-brand-charcoal border border-brand-dark-gray rounded-xl p-6 relative overflow-hidden">
              ...
              <button
                onClick={handleInquiry}
                className="w-full group bg-brand-red hover:bg-dark-red text-white text-xs font-bold py-3.5 rounded uppercase tracking-wider flex items-center justify-center gap-2 cursor-pointer transition-all active:scale-95 shadow-md shadow-brand-red/10 animate-[pulse_3s_infinite]"
              >
                <Send size={12} />
